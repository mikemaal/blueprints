 ...
 * element is retrieving the property {@link StringFactory#LABEL}. This property
...
class SparkseeElement implements Element {
	...
	public <T> T getProperty(final String key) {
		...
		com.sparsity.sparksee.gdb.Attribute adata = graph.getRawGraph()
				.getAttribute(attr);
		assert adata != null;

		com.sparsity.sparksee.gdb.Value v = new com.sparsity.sparksee.gdb.Value();
		Attribute at = graph.getRawGraph().getAttribute(attr);
		Object result = null;
		if (at.getDataType().equals(DataType.Text)) {
			TextStream ts = graph.getRawGraph().getAttributeText(oid, attr);
			if (!ts.isNull()) {
				StringBuilder str = new StringBuilder();			
				int len = 0;
				while ((len = ts.read(buf,CAPACITY)) > 0) {
			...
			if (!v.isNull()) {
				...
				case Double:
					...
					break;
				case Timestamp:
					result = v.getTimestamp();
					break;
				default:
					throw new UnsupportedOperationException(
							SparkseeTokens.TYPE_EXCEPTION_MESSAGE);
				}
			}
		}
		return (T) result;
	...
	public Set<String> getPropertyKeys() {
		...
		com.sparsity.sparksee.gdb.AttributeList alist = graph.getRawGraph()
				.getAttributes(oid);
		Set<String> attrKeys = new HashSet<String>();
		if(graph.getRawGraph().getType(graph.getRawGraph().getObjectType(oid)).getObjectType().equals(ObjectType.Node)){
			attrKeys.add(StringFactory.LABEL);
		}
	...
	public void setProperty(final String key, final Object value) {
		...
		if (key.equals(StringFactory.LABEL))
			throw new IllegalArgumentException(
					"Property key is reserved for all vertices and edges: "
							+ StringFactory.LABEL);
		graph.autoStartTransaction();
		...
		if (attr == com.sparsity.sparksee.gdb.Attribute.InvalidAttribute) {
			...
			if (value instanceof Boolean) {
				...
				datatype = ((com.sparsity.sparksee.gdb.Value) value)
						.getDataType();
			} else {
				throw new IllegalArgumentException(
						SparkseeTokens.TYPE_EXCEPTION_MESSAGE);
			...
			attr = graph.getRawGraph().newAttribute(type, key, datatype,
					com.sparsity.sparksee.gdb.AttributeKind.Basic);
